...
def create_bar_chart(ax, labels, values, colors, accent_color: str):
    bars = ax.barh(labels, values, color=colors, height=0.7, edgecolor='#606060', linewidth=0.8)
...
def save_figure(fig, path: str, logger):
    try:
        fig.savefig(path, facecolor=fig.get_facecolor(), bbox_inches='tight', dpi=120, transparent=False)
